...
type BoardPost struct {
	...
	// Side is what the AUTHOR does with Bitcoin, in the same vocabulary a swap
	// uses: LegSend means they pay BTC and receive ZNN, LegReceive the reverse.
	// A taker takes Opposite(). Stated as the author's side rather than as
	// "buy"/"sell" because "buying" names no asset without a convention, and the
	// two sides of this trade are both currencies.
	Side Leg `json:"side"`
	...
	// AmountSats is the Bitcoin side, exactly. Min/Max, when set, say the author
	// will do any size in that band -- so a taker names an amount and the two
	// settle on one before a swap exists. Empty Min/Max means the amount is the
	// amount.
	AmountSats int64 `json:"amountSats"`
	MinSats    int64 `json:"minSats,omitempty"`
	MaxSats    int64 `json:"maxSats,omitempty"`

	// ZenonAmt is the decimal amount as typed, matching Offer.ZenonAmt and
	// CreateParams.ZenonAmount. Kept as text for the reason normalDecimal exists:
	// a number that has been through a float is a number that has changed.
	ZenonAmt   string `json:"zenonAmt"`
	ZenonToken string `json:"zenonToken,omitempty"`

	// LockHours the author proposes for their own Bitcoin contract. Advisory:
	// what actually protects the swap is auditLegOrdering against the real
	// contract, not a number in an advertisement.
	LockHours int `json:"lockHours,omitempty"`

	// Addresses the author is willing to advertise. Both optional, and neither is
	// needed to trade -- the addresses that matter are exchanged inside the swap.
	// They are here because a bound wallet proof has to name one, and because a
	// counterparty who can see where the money will come from can look it up.
...
func (p BoardPost) validate() error {
	if p.Version != 1 {
		return fmt.Errorf("this is a version %d board post and this build reads version 1", p.Version)
	}
	...
	if p.Side != LegSend && p.Side != LegReceive {
		return fmt.Errorf("%q is not a side: it is %q or %q", p.Side, LegSend, LegReceive)
	}
	...
	if p.AmountSats <= 0 {
		return errors.New("a board post needs a Bitcoin amount above zero")
	}
	...
	if p.MinSats < 0 || p.MaxSats < 0 {
		return errors.New("a size band cannot be negative")
	}
	if p.MinSats > 0 && p.MaxSats > 0 && p.MinSats > p.MaxSats {
		return fmt.Errorf("the smallest size (%d sat) is larger than the largest (%d sat)",
			p.MinSats, p.MaxSats)
	}
	if p.MinSats > 0 && p.AmountSats < p.MinSats {
		return fmt.Errorf("the amount (%d sat) is below this post's own minimum (%d sat)",
			p.AmountSats, p.MinSats)
	}
	if p.MaxSats > 0 && p.AmountSats > p.MaxSats {
		return fmt.Errorf("the amount (%d sat) is above this post's own maximum (%d sat)",
			p.AmountSats, p.MaxSats)
	}
	if strings.TrimSpace(p.ZenonAmt) == "" {
		return errors.New("a board post needs a Zenon amount — it is half the trade")
	}
	if normalDecimal(p.ZenonAmt) == "" || strings.ContainsAny(p.ZenonAmt, "-eE") {
		return fmt.Errorf("%q is not a Zenon amount this can read", p.ZenonAmt)
	}
	if p.ZenonToken != "" {
		if _, err := znn.ParseTokenStandard(p.ZenonToken); err != nil {
			return err
		}
	}
	if p.ZnnAddr != "" {
		if _, err := znn.ParseAddress(p.ZnnAddr); err != nil {
			return err
		}
	}
...
func ReadPost(ev *NostrEvent, network string, now time.Time) (*Listing, error) {
	...
	for _, proof := range post.Proofs {
		...
		// verified".
		claimed := proof.Address
		switch proof.Scheme {
		case ProofBTC:
			if strings.TrimSpace(post.BtcAddr) != claimed {
				listing.Problems = append(listing.Problems,
					"its Bitcoin proof is for an address the post does not name")
				continue
			}
		case ProofZNN:
			if strings.TrimSpace(post.ZnnAddr) != claimed {
				listing.Problems = append(listing.Problems,
					"its Zenon proof is for an address the post does not name")
				continue
			}
		}
...
type Take struct {
	...
	// AmountSats is what the taker wants to do, within the post's band. Absent
	// means the post's headline amount.
	AmountSats int64 `json:"amountSats,omitempty"`

	// BtcAddr and ZnnAddr are where the TAKER wants each half of the trade paid.
	//
	...
	//
	// Of the two, the Zenon address is the one a swap cannot be built without —
	// an HTLC is created TO an address, so the author needs the taker's before
	// they can make their leg. The Bitcoin address is the taker's own payout and
	// the author never spends to it; it is carried so each side can see the
	// whole shape of what they are agreeing to, and so neither has to paste an
	// address into a chat window to be asked for it later.
	//
	// Optional in the wire format, required by the code that sends one — see
	// handleBoardTake. A take that predates this carries neither, and refusing
	// to READ those would hide requests the author can still answer.
	BtcAddr string `json:"btcAddr,omitempty"`
	ZnnAddr string `json:"znnAddr,omitempty"`
...
func (t Take) validate() error {
	if t.Version != 1 {
		return fmt.Errorf("this is a version %d take and this build reads version 1", t.Version)
	}
	if t.PostID == "" {
	...
	if t.AmountSats < 0 {
		return errors.New("a take cannot ask for a negative amount")
	}
	// Checked when present rather than required, so a take from a build that
	// predates carrying addresses still reads. An address that is present and
	// unparseable is a different thing entirely: it would reach the author as a
	// destination they might try to pay.
	if t.ZnnAddr != "" {
		if _, err := znn.ParseAddress(t.ZnnAddr); err != nil {
			return err
		}
	}
	if len(t.BtcAddr) > 128 || len(t.ZnnAddr) > 128 {
