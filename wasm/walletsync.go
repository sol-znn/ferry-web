...
func checkWalletAgainstSwap(ctx context.Context, a *API, mgr *Manager, req walletSyncReq,
	...
	// Which Zenon leg this action is about is decided by the action, not by the
	// caller: create and reclaim are always this user's own leg, and unlock is
	// always the counterparty's. A ZTS-against-ZTS swap has two Zenon legs, and
	// that mapping is what keeps them apart.
	dir := DirOut
	if req.Action == "unlock" {
		dir = DirIn
	}
	l, lerr := sw.znnLeg(dir)
	if lerr != nil {
		out.Problems = append(out.Problems, lerr.Error())
		return
	}
