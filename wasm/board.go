...
const (
	...
	// ProofSOL is a raw ed25519 signature over the same statement, from a
	// Solana wallet's `signMessage`.
	//
	// The same primitive as ProofZNN and a shorter check, because a Solana
	// address IS the public key: base58-decode it and verify. Zenon derives its
	// address from a hash of the key, so it needs the key carried alongside;
	// here the key cannot disagree with the address, because it is the address.
	//
	// What is assumed of the wallet: `signMessage(utf8Bytes(statement))` returns
	// a 64-byte detached signature over exactly those bytes. Phantom, Solflare
	// and Backpack all do. A wallet that wraps the message in a display prefix
	// would need its own scheme rather than a tweak to this one.
	ProofSOL = "sol-ed25519"
)
...
func VerifyProof(p WalletProof, boardPubKey, network string) error {
	...
	case ProofSOL:
		return verifySOLProof(p, statement)
	default:
		return fmt.Errorf("unknown wallet proof scheme %q", p.Scheme)
	}
}

// SchemeForChain is which proof a chain's address is claimed with. One per
// chain: the scheme is a property of the signature format, and a chain that
// gained a second would be two rows in `chains` rather than a list.
func SchemeForChain(c ChainID) string {
...
// ChainForScheme is the inverse, so a proof arriving off a relay can be told
// which chain it is about.
func ChainForScheme(scheme string) ChainID {
	for _, id := range ChainOrder {
		if chains[id].Proof == scheme {
			return id
		}
	}
	return ""
...
// verifySOLProof checks an ed25519 signature against the address that made it.
//
// Shorter than the Zenon case by exactly the step that cannot go wrong here: a
// Solana address IS the public key, so there is no separate key to carry and no
// derivation to check. A proof that verifies against the address in it is a
// proof about that address.
//
// The signature is hex like the Zenon one rather than base58 like everything
// else Solana writes, because the wallet hands back raw bytes and this app's
// convention for raw bytes is hex. The ADDRESS stays base58, which is the form
// its chain writes it in and the form a person would compare by eye.
func verifySOLProof(p WalletProof, statement string) error {
	addr := strings.TrimSpace(p.Address)
	pub, err := sol.ParsePubkey(addr)
	if err != nil {
		return err
	}
	// A public key that is not a curve point cannot have signed anything: it is
	// a program-derived address, and there is no key behind it. ed25519.Verify
	// would refuse it too, with a less useful sentence.
	sig, err := hex.DecodeString(strings.TrimSpace(p.Sig))
	if err != nil {
		return fmt.Errorf("that signature is not hex: %w", err)
	}
	...
	if !ed25519.Verify(ed25519.PublicKey(pub.Bytes()), []byte(statement), sig) {
		return fmt.Errorf("that signature does not verify against the statement %s should have "+
			"signed. A wallet signs with whichever account it has selected, so this is usually "+
			"the wrong account rather than a bad signature", addr)
	}
	return nil
}
