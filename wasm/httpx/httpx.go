...
//
// It exists for a size reason that is the difference between a usable page and
// an unusable one. Compiling `net/http` to WebAssembly costs about 8 MB raw /
// 2.2 MB gzipped, because the package unconditionally links crypto/tls, x509 and
// the HTTP/2 stack -- none of which run under GOOS=js, where the round-tripper
// hands everything to fetch() anyway. The linker cannot prune them: http.Client
// reaches Transport reaches tls.
//
// Calling fetch() directly takes the whole download from ~4.0 MB to ~2.9 MB
// gzipped, at the cost of one small file per build target.
//
...
// 8 MiB is far above any Esplora or Zenon response this app makes.
const MaxBody = 8 << 20
...
// message or a bare-value endpoint like Esplora's /blocks/tip/height.
...
// because the host is down, the certificate is bad, or the response lacked an
// Access-Control-Allow-Origin header -- telling a page the difference would leak
// information about the network it is on. All three arrive as the same opaque
// TypeError, so the message has to name all three: getting this wrong sends
// people to check a node that was never the problem.
const errCORS = "the browser blocked or could not complete this request. " +
	...
	"(Access-Control-Allow-Origin). Public Esplora instances do; a self-hosted node or a " +
