...
func sentCreate(t *testing.T, hash string) (*API, map[string]any, *Swap) {
	...
	sw := &Swap{ID: "a1b2c3d4", Role: RoleParticipant, Leg: LegReceive, State: StateAwaitingFunding}
	sw.Zenon.PeerAddress = "z1qqvwzz2xq7q5gwk6uhcddgrpxlfcyzc8rsu82s"
	sw.Zenon.AmountDisplay = "10"
...
func TestWalletSentRecordsTheIdEvenWhenItIsNotOnChainYet(t *testing.T) {
	...
	if stored.Zenon.HtlcID != publishedHash {
		t.Errorf("the HTLC id was not written to the swap: got %q, want %q",
			stored.Zenon.HtlcID, publishedHash)
	}
	if stored.Zenon.SelfAddress != testAddr {
		t.Errorf("the account that signed was not recorded: got %q", stored.Zenon.SelfAddress)
	}
}
...
func TestWalletSentRefusesACreateWithNoHash(t *testing.T) {
	node := notYetOnChain(t)
	api := &API{Store: NewStore(NewMemStorage())}
	if err := api.Store.Save(&Swap{ID: "a1b2c3d4", Role: RoleParticipant, Leg: LegReceive}); err != nil {
		t.Fatal(err)
	}
...
func TestWalletSentRecordsAnUnlock(t *testing.T) {
	...
	sw := &Swap{ID: "a1b2c3d4", Role: RoleInitiator, Leg: LegSend, State: StateFunded}
	sw.Zenon.HtlcID = publishedHash
	if err := api.Store.Save(sw); err != nil {
		t.Fatal(err)
	}
	...
	if stored.Zenon.UnlockHash != publishedHash {
		t.Errorf("the unlock was not recorded: got %q, want %q",
			stored.Zenon.UnlockHash, publishedHash)
	}
}
...
func TestWalletSentDoesNotRecordACreateAsAnUnlock(t *testing.T) {
	...
	if stored.Zenon.UnlockHash != "" {
		t.Errorf("a create was recorded as an unlock: %q", stored.Zenon.UnlockHash)
	}
}
