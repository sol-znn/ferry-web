...
func (sw *Swap) validate() error {
	...
	if sw.Out == nil || sw.In == nil {
		return errors.New("has fewer than two legs, so it does not describe a trade")
	}
	if err := CheckPair(sw.Out.Chain, sw.In.Chain, sw.Out.Token, sw.In.Token); err != nil {
		return err
	}
	if sw.Out.Dir != DirOut || sw.In.Dir != DirIn {
		return errors.New("has a leg whose direction disagrees with the slot it is in")
	}
	for _, l := range []*Leg{sw.Out, sw.In} {
...
// validate rejects a leg that could not have come out of this program.
//
// Structure rather than plausibility, exactly as above: every later code path
// assumes the chain-specific half of a leg is the one its chain names, and that
// a Bitcoin leg's key is internally consistent.
func (l *Leg) validate() error {
	...
	case ChainBTC:
		if l.Btc == nil {
			return errors.New("has a Bitcoin leg with no Bitcoin state")
		}
		k := l.Btc.Key
		if k == nil {
			return errors.New("has a Bitcoin leg with no key, so nothing in it could ever " +
				"spend a contract")
		}
		if len(k.Priv) != 32 {
			return fmt.Errorf("has a %d-byte private key, want 32", len(k.Priv))
		}
		if len(k.Pub) != 33 {
			return fmt.Errorf("has a %d-byte public key, want a 33-byte compressed one", len(k.Pub))
		}
		if len(k.PKH) != 20 {
			return fmt.Errorf("has a %d-byte pubkey hash, want 20", len(k.PKH))
		}
		// The three halves of the key have to agree, or the record names a
		// contract branch it cannot actually satisfy -- which is a discovery
		// worth making at import time rather than at refund time.
		if pub := k.PrivKey().PubKey().SerializeCompressed(); !bytes.Equal(pub, k.Pub) {
			return errors.New("has a public key that does not belong to its private key")
		}
		if !bytes.Equal(btcutil.Hash160(k.Pub), k.PKH) {
			return errors.New("has a pubkey hash that is not the hash of its public key")
		}
		if len(l.Btc.Contract) > 0 {
			if _, err := ParseContract(l.Btc.Contract); err != nil {
				return fmt.Errorf("has a contract this tool cannot parse: %w", err)
			}
		}
	case ChainZNN:
		if l.Znn == nil {
			return errors.New("has a Zenon leg with no Zenon state")
		}
	case ChainSOL:
		if l.Sol == nil {
			return errors.New("has a Solana leg with no Solana state")
		}
		if len(l.Sol.SwapID) != 64 {
			return errors.New("has a Solana leg whose swap id is not 32 bytes of hex")
		}
		if _, err := l.SwapIDBytes(); err != nil {
			return err
		}
	}
	return nil
