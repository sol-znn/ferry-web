...
func TestPostRoundTrips(t *testing.T) {
	...
	if listing.Post.AmountSats != post.AmountSats || listing.Post.ZenonAmt != post.ZenonAmt {
...
func TestBoardTagIsScopedToTheInstance(t *testing.T) {
	...
	take, err := SealTake(id, other.PubKey, Take{Version: 1, PostID: "abcd1234", Code: code})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostValidationRefusesIncoherentTerms(t *testing.T) {
	...
	for name, breakIt := range map[string]func(*BoardPost){
		"no amount":          func(p *BoardPost) { p.AmountSats = 0 },
		"negative amount":    func(p *BoardPost) { p.AmountSats = -1 },
		"no zenon amount":    func(p *BoardPost) { p.ZenonAmt = "  " },
		"negative zenon":     func(p *BoardPost) { p.ZenonAmt = "-5" },
		"unknown network":    func(p *BoardPost) { p.Network = "dogecoin" },
		"nonsense side":      func(p *BoardPost) { p.Side = Leg("sideways") },
		"nonsense role":      func(p *BoardPost) { p.Role = Role("bystander") },
		"nonsense status":    func(p *BoardPost) { p.Status = "haggling" },
		"inverted band":      func(p *BoardPost) { p.MinSats, p.MaxSats = 900, 100 },
		"amount below band":  func(p *BoardPost) { p.MinSats = p.AmountSats + 1 },
		"amount above band":  func(p *BoardPost) { p.MaxSats = p.AmountSats - 1 },
		...
		"bad zenon address":  func(p *BoardPost) { p.ZnnAddr = "z1nonsense" },
		"bad zenon token":    func(p *BoardPost) { p.ZenonToken = "zts1nonsense" },
		"future version":     func(p *BoardPost) { p.Version = 2 },
	} {
...
func TestReadPostWillNotBadgeAnUnrelatedAddress(t *testing.T) {
	...
	post.BtcAddr = "bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4"
...
func TestTakeReachesOnlyItsRecipient(t *testing.T) {
	...
	if got.Take.AmountSats != 500_000 {
		t.Fatalf("amount is %d, want 500000", got.Take.AmountSats)
	}
...
func TestTakeRefusesWhatItShould(t *testing.T) {
	...
	t.Run("half a code", func(t *testing.T) {
		...
		if _, err := SealTake(taker, maker.PubKey, Take{
			Version: 1, PostID: "abcd1234", Code: "ABCD",
		}); err == nil {
	...
	t.Run("your own post", func(t *testing.T) {
		if _, err := SealTake(taker, taker.PubKey, Take{
			Version: 1, PostID: "abcd1234", Code: code,
		}); err == nil {
...
func TestBoardRefusesAnAddressThatIsNotProven(t *testing.T) {
	...
	publish := func(t *testing.T, api *API, btc, znn string) error {
		...
		body, err := json.Marshal(map[string]any{
			"side": LegSend, "role": RoleInitiator,
			"amountSats": 100_000, "zenonAmt": "1200",
			"btcAddr": btc, "znnAddr": znn,
