...
type postReq struct {
	...
	// Give is what the author funds and Want what they receive, in the same
	// shape the post itself carries -- there is nothing to translate, and a
	// translation layer here would be a second place the two could disagree.
	Give PostLeg `json:"give"`
	Want PostLeg `json:"want"`

	LockHours int `json:"lockHours,omitempty"`
	// Addrs are the author's addresses, keyed by chain. Only the chains this
	// offer settles on are looked at; anything else is dropped rather than
	// published, because an address nobody is trading with is a claim with no
	// reader.
...
func handleBoardPublish(_ context.Context, a *API, body []byte) (any, error) {
	...
	// Only the chains this offer settles on. An address filed under a chain the
	// trade does not touch is not published: it would be a claim nobody reading
	// this post can act on, and ReadPost refuses to badge it anyway.
	post.Addrs = map[string]string{}
	for _, c := range post.Chains() {
		if addr := strings.TrimSpace(req.Addrs[string(c)]); addr != "" {
			post.Addrs[string(c)] = addr
		}
	}
	...
	for _, c := range post.Chains() {
		if post.Addr(c) == "" {
			return nil, fmt.Errorf("an offer has to name an address on every chain it settles "+
				"on, and this one has no %s address — it is where that half of the trade gets "+
				"paid, and a taker has no other way to learn it. Prove an address on each chain "+
				"and they fill themselves in", ChainLabel(c))
		}
	}
	...
	if err := requireProven(id, post.Addrs, post.Chains()); err != nil {
		return nil, err
	}
...
// chainsNamed pairs each chain a trade settles on with the address given for it
// and the scheme that proves one. A chain with no address named asks for no
// proof: there is nothing to claim.
func chainsNamed(addrs map[string]string, need []ChainID) []chainAddr {
	var out []chainAddr
	for _, c := range need {
		a := strings.TrimSpace(addrs[string(c)])
		if a == "" {
			continue
		}
		out = append(out, chainAddr{name: ChainLabel(c), scheme: SchemeForChain(c), addr: a})
	}
	return out
}
...
func requireProven(id *BoardIdentity, addrs map[string]string, need []ChainID) error {
	for _, c := range chainsNamed(addrs, need) {
...
func proofsFor(id *BoardIdentity, post BoardPost) []WalletProof {
	...
	for _, c := range post.Chains() {
		...
		if p := id.Binding(SchemeForChain(c)); p != nil && p.Address == addr {
			out = append(out, *p)
		}
	}
	return out
...
func handleBoardTake(_ context.Context, a *API, body []byte) (any, error) {
	var req struct {
		...
		// Amount is the size the taker wants, when the post advertises a band.
		// In the unit of the leg its AUTHOR funds, which is the one a band can
		// be set on.
		Amount string `json:"amount,omitempty"`
		Note   string `json:"note,omitempty"`
		// Addrs is where this taker wants each half paid, keyed by chain. Sealed
		// to the author with the rest of the take -- see Take.
		Addrs map[string]string `json:"addrs,omitempty"`
		// Chains is which chains the offer settles on, so the same per-chain
		// rule the publish side applies can be applied from this end.
	...
	need := req.Chains
	if len(need) == 0 {
		return nil, errors.New("a take has to say which chains the offer settles on, so the " +
			"addresses in it can be checked against the proofs for those chains")
	}
	addrs := map[string]string{}
	for _, c := range chainsOf(need...) {
		addr := strings.TrimSpace(req.Addrs[string(c)])
		if addr == "" {
			return nil, fmt.Errorf("taking this offer has to say where you want your %s paid — "+
				"prove an address on each chain it settles on and they fill themselves in",
				ChainLabel(c))
		}
		addrs[string(c)] = addr
	}
	...
	if err := requireProven(id, addrs, chainsOf(need...)); err != nil {
		return nil, err
	}
	code := req.Code
