...
func TestOfferRoundTripCarriesNoSecrets(t *testing.T) {
	o := &Offer{
		...
		Give: OfferLeg{
			Chain: ChainBTC, Amount: "100000", PKH: hex.EncodeToString(make([]byte, 20)),
		},
	...
	if back.SecretHash != o.SecretHash || back.Give.PKH != o.Give.PKH ||
		back.Give.Chain != o.Give.Chain || back.Take.Chain != o.Take.Chain ||
		back.Give.Amount != o.Give.Amount || back.Take.Amount != o.Take.Amount {
		t.Errorf("round trip mismatch: %+v vs %+v", back, o)
	}
	// A v1 string is refused rather than guessed at: it does not say which
	// chain each half is on, and inventing that is how somebody funds the wrong
	// contract.
	if _, err := DecodeOffer("swapoffer1:eyJ2IjoxfQ"); err == nil {
		t.Error("a version 1 offer was accepted")
	}
