...
func fillFromSwap(msg *SessionMessage, sw *Swap) error {
	...
	// Which leg a hand-off is about is a property of the value, not of the
	// message, and there is exactly one candidate for each: a pubkey hash goes
	// to whoever will BUILD the contract, a contract comes from whoever built
	// it, an HTLC id from whoever created the entry, a funding txid from
	// whoever paid. Deriving them here rather than taking a direction off the
	// wire is what stops a message about the wrong leg being applied.
	switch msg.Type {
	case "pkh":
		// The receiving side sends their pubkey hash, so the funding side can
		// put it in the redeem branch.
		l := sw.In
		if l == nil || l.Chain != ChainBTC || l.Btc == nil || l.Btc.Key == nil {
			return errors.New("this swap has no incoming Bitcoin leg, so there is no pubkey " +
				"hash to send")
		}
		msg.Leg = string(ChainBTC)
		msg.PkhHex = hex.EncodeToString(l.Btc.Key.PKH)
	case "contract":
		l := sw.Out
		if l == nil || l.Chain != ChainBTC || l.Btc == nil || len(l.Btc.Contract) == 0 {
			return errors.New("this swap has no Bitcoin contract of yours yet — build it first")
		}
		msg.Leg = string(ChainBTC)
		msg.ContractHex = hex.EncodeToString(l.Btc.Contract)
	case "zenon":
		l, err := sw.znnLeg(DirOut)
		if err != nil || l.Znn.HtlcID == "" {
			return errors.New("this swap has no Zenon HTLC of yours yet")
		}
		msg.Leg = string(ChainZNN)
		msg.HtlcID = l.Znn.HtlcID
		msg.ZenonAddr = l.SelfAddr
	case "solana":
		l, err := sw.solLeg(DirOut)
		if err != nil {
			return errors.New("this swap has no Solana leg of yours")
		}
		msg.Leg = string(ChainSOL)
		msg.SolProgram = l.Sol.ProgramID
		msg.SolSwapID = l.Sol.SwapID
		msg.SolAddr = l.SelfAddr
	case "funded":
		l := sw.Out
		if l == nil || l.Chain != ChainBTC || l.Btc == nil || l.Btc.Funding == nil {
			return errors.New("your Bitcoin contract is not funded yet")
		}
		msg.Leg = string(ChainBTC)
		msg.FundingTxID = l.Btc.Funding.TxID
