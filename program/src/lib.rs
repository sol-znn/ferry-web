...
fn create(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    if amount == 0 {
        msg!("ferry: amount must be non-zero");
    ...
    if timelock <= now {
        msg!("ferry: timelock {} is not in the future (now {})", timelock, now);
        return Err(ProgramError::InvalidArgument);
    }

    let (expected, bump) = Pubkey::find_program_address(&[PDA_PREFIX, &swap_id], program_id);
    if expected != *escrow.key {
        msg!("ferry: escrow account is not the PDA for this swap id");
        return Err(ProgramError::InvalidArgument);
    }
    ...
    if !escrow.data_is_empty() || escrow.owner != &system_program::ID {
        msg!("ferry: swap id is already in use");
    ...
    if escrow.lamports() < lamports {
        msg!("ferry: the escrow does not hold the amount plus rent");
    ...
    msg!("ferry: created, amount {} timelock {}", amount, timelock);
    Ok(())
}
...
fn redeem(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    if *receiver.key != state.receiver {
        msg!("ferry: receiver account does not match the escrow");
        return Err(ProgramError::InvalidArgument);
    }
    if *initiator.key != state.initiator {
        msg!("ferry: initiator account does not match the escrow");
        return Err(ProgramError::InvalidArgument);
    }
    ...
    if now >= state.timelock {
        msg!("ferry: expired at {} (now {})", state.timelock, now);
        return Err(ProgramError::InvalidArgument);
    }
    ...
    if hashv(&[preimage]).to_bytes() != state.hashlock {
        msg!("ferry: preimage does not match the hashlock");
        return Err(ProgramError::InvalidArgument);
    }

    close_to(escrow, state.amount, receiver, initiator)?;
    msg!("ferry: redeemed {} lamports", state.amount);
    Ok(())
}
...
fn refund(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    let now = Clock::get()?.unix_timestamp;
    if now < state.timelock {
        msg!("ferry: not refundable until {} (now {})", state.timelock, now);
        return Err(ProgramError::InvalidArgument);
    }

    close_to(escrow, 0, initiator, initiator)?;
    msg!("ferry: refunded {} lamports", state.amount);
    Ok(())
}
