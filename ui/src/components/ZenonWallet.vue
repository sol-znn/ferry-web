...
import type {LegView, Swap, WalletAction, WalletBlockPlan, WalletSync} from '@/types'
...
const props = defineProps<{
  swap: Swap
  /** The Zenon leg this call is about. A ZTS-against-ZTS swap has two of them,
   *  so the panel is told which rather than searching for one. */
  leg: LegView
...
watch([() => props.swap.id, () => props.leg.expiry, () => props.leg.znn?.htlcId], () => {
  plan.value = null
  sync.value = null
})
