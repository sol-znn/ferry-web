...
// have decided to do.
//
// The direction label is written from the READER's side, not the author's. Every
// field on a post is stored from the author's point of view — `side: send` means
// THEY pay Bitcoin — and a board that showed that verbatim would be a board
// every reader has to invert in their head, every row, forever.
...
const direction = computed(() => directionFor(post.value))
...
const headline = computed(() =>
  direction.value === 'buy-znn'
    ? {verb: 'Buy ZNN', detail: 'you pay BTC, they pay ZNN'}
    : {verb: 'Sell ZNN', detail: 'you pay ZNN, they pay BTC'},
)
 ...
 */
const answerable = computed(() => Boolean(post.value.btcAddr && post.value.znnAddr))
 ...
 */
const missing = computed(() => missingChains(chainsForPost(post.value), props.proven ?? []))
</script>
    ...
    <div class="grid min-w-0 gap-1.5">
      <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
        <span
          ...
          :class="direction === 'buy-znn' ? 'text-success' : 'text-warning'"
        >
          {{ headline.verb }}
        </span>
      ...
      <div class="flex flex-wrap items-baseline gap-x-4 gap-y-1">
        <span class="font-mono text-sm">{{ btc(post.amountSats) }}</span>
        <ArrowRightLeftIcon class="size-3 shrink-0 text-muted-foreground" />
        <span class="font-mono text-sm">
          {{ post.zenonAmt }} {{ tokenName(post.zenonToken) }}
        </span>
        <span class="font-mono text-xs text-muted-foreground">
          {{ formatRate(rate(post)) }} ZNN/BTC
          <span class="opacity-70">· {{ formatRate(satsPerZnn(post)) }} sat/ZNN</span>
        </span>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
        ...
        <span
          v-if="hasProof(listing, 'btc-ecdsa')"
          class="inline-flex items-center gap-1 text-success"
        >
          <BadgeCheckIcon class="size-3" />
          {{ shortAddr(post.btcAddr ?? '') }}
        </span>
        <span
          v-if="hasProof(listing, 'znn-ed25519')"
          class="inline-flex items-center gap-1 text-success"
        >
          <BadgeCheckIcon class="size-3" />
          {{ shortAddr(post.znnAddr ?? '') }}
        </span>
    ...
    <div class="flex items-center gap-2 sm:justify-end">
      <template v-if="takeable">
        ...
        <InfoTip v-else-if="missing.length" label="Why Take is disabled">
          <p>
            This offer settles on {{ chainList(chainsForPost(post)) }}, and you have not proven
