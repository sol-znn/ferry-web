 ...
 * tokens needs Zenon and has no Bitcoin leg to prove. See chains.ts — the
 * requirement is derived from the post, so a pair this file has never heard of
 * gates itself correctly.
 *
...
export function useChainProofs() {
  const {btcBound, znnBound} = useBoardIdentity()
  ...
  return {proven, addresses, addressFor, missingFor, readyFor, messageFor, canPostAny, postBlockers}
}
