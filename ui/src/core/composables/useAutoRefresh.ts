...
function onChain(sw: Swap): boolean {
  return Boolean(sw.contractAddr) && sw.network === stored.value.network
}
 ...
 */
function verifyDue(sw: Swap): boolean {
  return Boolean(sw.zenon?.htlcId) && !sw.zenon?.verified && Boolean(sw.zenon?.verifyPending)
}
...
async function syncSwap(sw: Swap): Promise<boolean> {
  ...
  if (hasZenon.value && verifyDue(now)) {
    try {
      ...
      const r = await api.verifyZenon(sw.id, now.zenon.htlcId ?? '', settings.value)
