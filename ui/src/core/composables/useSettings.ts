...
const body = computed<Settings>(() => {
  ...
  if (stored.value.solRpc) out.solRpc = stored.value.solRpc
  if (stored.value.solProgram) out.solProgram = stored.value.solProgram
...
/** The Solana deployment a new leg would be created on. Blank means the one
 *  baked into the module at build time, which the config call reports back —
 *  and blank on both is a build that cannot create a Solana leg at all. */
const solProgram = computed(() => stored.value.solProgram ?? '')
...
export function useSettings() {
  return {stored, body, effectiveEsplora, hasZenon, solProgram, relayList, save, reload}
}
