...
import type {ChainAgreement, ChainId, Role, SessionMessage, SessionRoom} from '@/types'
 ...
 * moment the page opens; the counterparty's real `swapoffer2` arrives over the
...
export interface PendingLeg {
  chain: ChainId
  token?: string
  /** As typed, in the chain's own unit. Scaled where a size band meant the two
   *  sides settled on something other than the headline amount. */
  amount: string
  /** Theirs, when the post or the take advertised one. */
  peerAddr?: string
}

export interface PendingTrade {
  /** Both legs, already flipped where they needed flipping: what THIS browser
   *  funds, and what it receives. */
  out: PendingLeg
  in: PendingLeg
  role: Role
  /** Zero leaves the duration to Go, which derives it from the role. */
  lockHours: number
 ...
 * form in without a signature behind it. `swapoffer2` is what pins a term.
 */
const peerAddresses = ref<Partial<Record<ChainId, string>> | null>(null)
