...
import {chainsForPost, type ChainId} from '@/core/chains'
...
async function take(
  listing: Listing,
  opts: {amount?: string; note?: string; addrs: Partial<Record<ChainId, string>>},
  ...
  busy.value = true
  try {
    // The size the taker wants, in the unit of the leg the AUTHOR funds --
    // which is the leg a band can be set on.
    const amount = opts.amount?.trim() || post.give.amount
    ...
    // one that already knows every proof is held.
    const {event, room} = await api.boardTake({
      author: listing.author,
      postId: post.id,
      note: opts.note,
      chains: chainsForPost(post),
      addrs: opts.addrs,
      amount,
    })
    publishAll([event])
    ...
    // The taker funds the author's `want` and receives the author's `give`.
    session.pendingTrade.value = {
      out: {
        chain: post.want.chain,
        token: post.want.token,
        amount: scaled(post.want.amount, amount, post.give.amount),
        peerAddr: post.addrs?.[post.want.chain] ?? '',
      },
      in: {
        chain: post.give.chain,
        token: post.give.token,
        amount,
        peerAddr: post.addrs?.[post.give.chain] ?? '',
      },
...
async function accept_(inbound: InboundTake): Promise<boolean> {
  ...
  try {
    ...
    // covers that.
    const p = post.post
    const amount = inbound.take.amount?.trim() || p.give.amount
    session.pendingTrade.value = {
      out: {
        chain: p.give.chain,
        token: p.give.token,
        amount,
        peerAddr: inbound.take.addrs?.[p.give.chain] ?? '',
      },
      in: {
        chain: p.want.chain,
        token: p.want.token,
        amount: scaled(p.want.amount, amount, p.give.amount),
        peerAddr: inbound.take.addrs?.[p.want.chain] ?? '',
      },
      role: p.role,
    ...
    void session.send(
      {
        type: 'addresses',
        btcAddr: p.addrs?.btc ?? '',
        zenonAddr: p.addrs?.znn ?? '',
        solAddr: p.addrs?.sol ?? '',
      },
      'where to pay you',
    )
    session.say('sys', `Accepted a take on post ${p.id}. Agree the swap in this session.`)
    return true
  } catch (e) {
    error.value = e instanceof Error ? e.message : String(e)
    return false
 ...
 */
function scaled(other: string, want: string, headline: string): string {
  const quoted = Number(other)
  const w = Number(want)
  const h = Number(headline)
  if (![quoted, w, h].every((n) => Number.isFinite(n) && n > 0)) return other
  if (w === h) return other
  const out = (quoted * w) / h
  if (!Number.isFinite(out) || out <= 0) return other
  return String(Number(out.toFixed(9)))
}

/** The other side of a role. A board post states the AUTHOR's, so a taker takes
 *  the opposite; getting it backwards is a swap where both sides think they are
 *  the initiator. */
