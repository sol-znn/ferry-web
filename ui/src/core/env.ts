...
export const INSTANCE_DEFAULTS: Record<FerryEnv, Settings> = {
  dev: {
    ...
    solRpc: 'http://127.0.0.1:8899',
  },
  prod: {
    network: 'mainnet',
    btcEsplora: '',
    znnUrl: '',
    // Blank, and it resolves to a public endpoint rather than to nothing --
    // unlike the Zenon side. The asymmetry is real: an escrow is checked
    // against a PDA the module derives itself, so a node that lies about the
    // account is caught by the derivation rather than believed.
    solRpc: '',
  },
}
