...
export const api = {
  ...
  /**
   * Check one of this swap's Zenon entries against the terms it committed to.
   *
   * `dir` names WHICH leg, because a ZTS-against-ZTS swap has two of them and an
   * entry checked against the wrong half's terms is a check that means nothing.
   * It defaults to the incoming leg, which is what every other pair means by
   * "their HTLC".
   */
  verifyZenon: (id: string, htlcId: string, settings: Settings, dir: Dir = 'in') =>
    wasmCall<{swap: Swap; error?: string}>('zenon', {id, htlcId, dir, settings}),
   ...
   */
  findZenon: (id: string, settings: Settings, dir: Dir = 'in') =>
    wasmCall<{swap: Swap; error?: string}>('zenonFind', {id, dir, settings}),

  /**
   * Build one Solana instruction for this swap: the create that funds an
   * outgoing leg, the redeem that claims an incoming one, or the refund that
   * takes an outgoing one back.
   *
   * What comes back is program id, accounts and data — never a transaction and
   * never a signature. The page assembles it, the user's wallet shows it and
   * signs it, and the page submits through the node named in Node settings. The
   * same split the Zenon leg makes with the Syrius extension, and for the same
   * reason: everything that decides whether a step is safe happens in Go.
   *
   ...
   */
  solInstruction: (id: string, action: SolAction, from: string, settings: Settings) =>
    wasmCall<{
      ...
      rentLamports?: number
      rent?: string
    }>('solInstruction', {id, action, from, settings}),

  /**
   * Record a signature the wallet produced, then re-read the chain.
   *
   * A signature is not evidence on its own — a transaction can be dropped, and
   * one that landed can have failed — so a refresh is what decides.
   */
  solSent: (id: string, action: SolAction, signature: string, settings: Settings) =>
    wasmCall<{swap: Swap; error?: string}>('solSent', {id, action, signature, settings}),
  ...
  boardTake: (req: {
    ...
    amount?: string
    /** Which chains the offer settles on, and where this taker wants each half
     *  paid. Go refuses a take that would reach an author with nowhere to pay,
     *  and one carrying an address this browser has not proven. */
    chains: ChainId[]
