...
declare global {
  interface Window {
    ...
    /**
     * Where the development build's testkit puts a stub. It cannot use the name
     * above: an installed Syrius defines `window.zenon` as non-writable and
     * non-configurable, so there is no slot to take. Read only on the
     * development instance, where the expression folds away at build time --
     * see core/testkit.ts.
     */
    __ferryZenon?: Partial<ZenonProvider>
  }
}
 ...
 */
/**
 * The instance, read from the build-time constant rather than through
 * `@/core/env`.
 *
 * This file deliberately imports nothing: scripts/wallet-provider.mjs loads it
 * as it ships, in Node, with no bundler to resolve an alias and nothing to
 * substitute a define. `typeof` therefore does the work -- under the bundler the
 * name is replaced by a literal and the whole test folds to false on production,
 * and under Node it is simply not defined.
 */
declare const __FERRY_ENV__: string | undefined
const devBuild = typeof __FERRY_ENV__ !== 'undefined' && __FERRY_ENV__ === 'dev'

function provider(): ZenonProvider | null {
  if (typeof window === 'undefined') return null
  const p = devBuild && window.__ferryZenon ? window.__ferryZenon : window.zenon
