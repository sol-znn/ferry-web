...
import {CHAINS, PAIRS, chainList, chainsForPost} from '@/core/chains'
 ...
 */
async function confirmTake(opts: {amount: string; note: string}) {
  ...
  const room = await board.take(listing, {...opts, addrs: proofs.addrsFor(need)})
  if (room) {
    ...
    <div class="grid gap-3 rounded-lg border border-border p-3">
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="grid gap-1.5">
          <Label class="text-ledger text-muted-foreground">Trade</Label>
          <Select
            :model-value="filters.pair"
            @update:model-value="(v) => (filters.pair = String(v))"
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="any">every pair</SelectItem>
        ...
        <div class="grid gap-1.5">
          <Label class="text-ledger text-muted-foreground">I would pay</Label>
          <Select
            :model-value="filters.giving"
            @update:model-value="(v) => (filters.giving = v as BoardFilters['giving'])"
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="any">either side</SelectItem>
              <SelectItem v-for="c in CHAINS" :key="c.id" :value="c.id">
                {{ c.label }}
              </SelectItem>
            </SelectContent>
          </Select>
        ...
        <div class="grid gap-1.5">
          <Label class="flex items-center gap-1.5 text-ledger text-muted-foreground">
            Sort by
            <InfoTip label="How 'best rate' knows what it is comparing">
              <p>
                A rate in one pair is in different units from a rate in another, and within one pair
                the two directions want opposite ends of the same number. So best rate sorts only
                once you have chosen a trade AND which side you would fund; otherwise it falls back
                to newest rather than ordering by a number that means something different in
                adjacent rows.
              </p>
            </InfoTip>
          </Label>
